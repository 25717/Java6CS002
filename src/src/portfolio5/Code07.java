...
/*
 * The code then filters the Football objects in the parallel stream to keep only those with total points greater than or equal to 60 using the filter method
 * */
...
public class Code07 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("\nUsing Parallel Stream");
    Stream<Football> parallelStream = footballList.parallelStream();
    parallelStream.filter(f -> f.getTotalPoints() >= 60).sorted((f1, f2) -> f2.getGoalsForward() - f1.getGoalsForward()).forEach(System.out::println);
  }
}